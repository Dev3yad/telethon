...
@iqthon.iq_cmd(
    pattern="اسكرين ([\s\S]*)",
    command=("اسكرين", plugin_category),
    info={
...
async def _(event):
    ...
    try:
        ...
        await event.edit("جاري فتح جوجل كروم لي دخول الموقع")
        ...
        if not caturl:
            ...
            caturl = url(inputstr)
        if not caturl:
            return await catevent.edit("الرابط خطأ او غير مدعوم")
        driver.get(inputstr)
        ...
        await catevent.edit("جاري اغلاق جوجل كروم")
        driver.close()
        message_id = await reply_id(event)
        end = datetime.now()
        ms = (end - start).seconds
        hmm = f"**الموقع : **{input_str} \n**الوقت :** `{ms} ثانيا`"
